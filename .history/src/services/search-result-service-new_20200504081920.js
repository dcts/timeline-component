  ...
  buildBinObject(dateStr, category, scope) {
    ...
    switch (scope) {
      ...
      case "W":
        const year = ""; // "2001"
        const week = ""; // "W52"
        binObject.binTitle       = category.split("-")[1]; // W52
        binObject.tooltip        = category.replace("-", " "); // 1996 W52
        binObject.selectionStart = category.replace("-", " ");
        binObject.selectionEnd   = category.replace("-", " ");
        binObject.seperator      = false;
        break;
      case "D":
        binObject.binTitle = "";
